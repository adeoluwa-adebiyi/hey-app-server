...
import { CreateChatRoomUsecase, DeleteChatRoomUsecase, GetChatRoomByRoomKeyUseCase } from "../../domain/usecases/create-chatroom.usecase";
...
router.get("/roomKey/:roomKey", async(req: Request, res: Response)=>{
    ...
    try{
        const response = await new GetChatRoomByRoomKeyUseCase().execute({roomKey});
        res.send({
            data:{
                ...response
            }
