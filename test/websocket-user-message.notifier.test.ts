    ...
    it("Should send message via websocket to user", (done) => {
        ...
        setTimeout(() => {
            ...
            new Promise(async (resolve, reject) => {
                    ...
                    try {
                        ...
                        socket.on("authenticated", (msg: any) => {
                            ...
                            if (serialized.type === "error")
                                ...
                                userMessageNotifier.notifyUser({ id: user.id }, data).then(() => {

                                    socket.on(WS_CHATROOM_MESSAGE_NOTIFY_EVENT, (msg: any) => {
                ...
                expect(JSON.stringify(serialized)).to.equal(JSON.stringify(data));
                done();
            }).catch(e => (done(e)));
        }, 5000);
    });
